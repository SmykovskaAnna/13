...
public class OrderController {
    ...
    public String viewOrderStatus(@PathVariable Long id, Principal principal, Model model) {
        User user = userRepo.findByUsername(principal.getName());
        Order order = orderRepo.findById(id).orElse(null);
    ...
    public String supportPage() {
        // Displays the support page for the user
        return "support";
    }

    @PostMapping("/order/status/next/{id}")
    public String nextOrderStatus(@PathVariable Long id, Principal principal) {
        // Get the currently authenticated user
        User user = userRepo.findByUsername(principal.getName());
        ...
        if (order != null && order.getUser().getId().equals(user.getId())) {
            ...
            // Update the order status to the next step in the process: "Placed" -> "Paid" -> "Delivered"
